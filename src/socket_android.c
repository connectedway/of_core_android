      ...
      else
	{
	  if (socktype == SOCKET_TYPE_DGRAM)
	  ...
	  else if (socktype == SOCKET_TYPE_STREAM)
	    {
	       ...
	       * SMB read on a vanished peer pins the calling thread in
	       * D state; on Android that pins the process, and
	       * ActivityManager can no longer kill it — the app becomes
	       * un-relaunchable until reboot. See openfiles#20.
	       *
  ...
  if (sock != OFC_NULL)
    {
      ...
      android_status = getsockname (sock->socket, local_sockaddr, 
				  &local_sockaddr_size) ;
      if (android_status == 0)
	{
	  ...
	  android_status = getpeername (sock->socket, remote_sockaddr, 
